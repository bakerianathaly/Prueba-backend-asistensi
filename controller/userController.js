...
async function signUp(req, res, done){
    ...
    var existingUser = await userModel.findOne({ email: data.email}).exec() //Consulta para ver si el correo ya existe 
    var existingUsername = await userModel.findOne({ username: data.username}).exec() //Consulta para ver si alguno registro en la BD tiene el username
...
async function loggedIn(req, res, done){
    var data = req.body //Datos de el usuario que vienen en el cuerpo de la petición
    var user = await userModel.findOne({username: data.username}).exec() //Consulta para ver si el usuario existe 
...
async function updateUser(req, res, done){
    ...
    var existingUser = await userModel.findById(id).exec() //Consulta para ver si el usuario existe 
...
async function deleteUser(req, res, done){
    var data = req.body //Datos de el usuario que vienen en el cuerpo de la petición
    ...
    if(!existingUser){
        ...
        try{
            ...
            userModel.findByIdAndRemove(existingUser._id).exec((err, doc) => {
                if (err) {
